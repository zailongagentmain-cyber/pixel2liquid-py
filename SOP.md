...
**Step 1: 观察**
```
现象：Manifest 显示 downloaded=66，但磁盘有 164 个文件
```

**Step 2: 分析**
```
假设 A：下载失败
验证：检查 skipped 文件是否实际存在 → 450 个存在
假设 B：多次运行累积
验证：检查目录结构 → 发现 assets_test/, assets_test4/ 等多个目录
结论：多次运行导致 manifest 记录分散，skipped = 已存在不重下 ≠ 未下载
```

**Step 3: 修复**
```
方案 A：同步 manifest（扫描实际文件，更新 manifest 状态）
方案 B：改进状态语义（明确 skipped vs downloaded）
决定：先做方案 A，立即可用；方案 B 改进架构
```

**Step 4: 预防**
```
添加到 SOP：每次下载后必须验证 manifest 一致性
添加 manifest doctor 命令：检查并修复不一致
```

---

### Manifest Doctor 命令规范

```python
def manifest_doctor(manifest_path: str, assets_dir: str):
    """
    检查 manifest 和实际文件的一致性

    返回：
    - missing_files: manifest 记录存在但文件不存在
    - missing_records: 文件存在但 manifest 没有记录
    - status_mismatch: 状态与实际不符（如 skipped 但文件存在）
    """
```
