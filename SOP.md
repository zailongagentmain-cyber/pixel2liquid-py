...
### 4. 中间结果必须保存
- 每个 Phase/功能完成后，**必须保存中间结果**到文件
- 保存的目的是：后续测试不需要重复执行前面的功能
- 保存内容包括：爬取状态、解析结果、raw_html（如获取了）等
- 文件命名规范：`<项目名>/<功能名>_cache.json` 或放在 `cache/` 目录
- 禁止：只保存在内存中，session 结束后丢失

**示例**：
```bash
# Phase 1 爬取完成后，保存到 cache/
cache/
├── crawl_state.json      # 爬取状态
├── pages/                # HTML 文件（可选）
└── manifest.json         # URL → 文件路径映射
```
